...
#		standard=get_standard
	...
	resources:
		mem_mb = 2000
	shell:
		"bash src/custom/evals/evaluate_macs2.sh -i {wildcards.sample} -s src/standards/ENCFF885FQN.bed -t {threads} > {output}"
...
#		standard=get_standard
	...
	resources:
		mem_mb = 2000
	shell:
		"bash src/custom/evals/evaluate_homer.sh -i {wildcards.sample} -s src/standards/ENCFF885FQN.bed -t {threads} > {output}"
...
#		standard=get_standard
	...
	resources:
		mem_mb = 2000
	shell:
		"bash src/custom/evals/evaluate_otron.sh -i {wildcards.sample} -s src/standards/ENCFF885FQN.bed -t {threads} > {output}"
