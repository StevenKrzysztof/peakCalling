...
threshold() {
    counts=$1
    predicted_truth=$(cat $sample_file_handle | awk -v s=$1 '$21 <= s')
    predicted_false=$(cat $sample_file_handle | awk -v s=$1 '$21 > s')
...
file=$(find data/lanceotron/runningData -name "${sample}*Peak") # this workaround accommodates for narrowPeak and broadPeak inputs
...
sample_file=$(awk -v OFS='\t' '{print $0,10**-$20}' $file | grep -vi "inf" | sort -rg -k21)
sample_pvals=$(echo "$sample_file" | cut -f21 | uniq)
sample_file_handle="data/evaluate_models/lanceOtron_${sample}.tmp.txt" # create tmp file to threshold from
